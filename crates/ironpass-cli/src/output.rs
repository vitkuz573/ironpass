use console::style;
use ironpass_api_client::models::StoredSubscription;
...
fn display_width(s: &str) -> usize {
    s.chars()
        .map(|c| if unicode_is_wide(c) { 2 } else { 1 })
        .sum()
}
...
pub fn print_nodes_table(nodes: &[ProxyNode], sub: &Subscription) -> io::Result<()> {
    ...
    writeln!(
        handle,
        "  Fetched:    {}",
        sub.fetched_at.format("%Y-%m-%d %H:%M:%S UTC")
    )?;
    if let Some(used) = sub.traffic_used {
        writeln!(
            handle,
            "  Traffic:    {} used",
            bytesize::ByteSize(used).display()
        )?;
    }
    if let Some(total) = sub.traffic_total {
        writeln!(
            handle,
            "    Total:    {}",
            bytesize::ByteSize(total).display()
        )?;
    }
...
pub fn print_subscriptions_api(subs: &[StoredSubscription], detailed: bool) {
    ...
    for (i, sub) in subs.iter().enumerate() {
        ...
        if detailed {
            println!(
                "     Added:    {}",
                sub.added_at.format("%Y-%m-%d %H:%M UTC")
            );
