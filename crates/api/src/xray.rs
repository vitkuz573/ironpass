...
fn build_vless_outbound(node: &ProxyNode) -> anyhow::Result<Value> {
    ...
    user.insert("encryption".into(), node.encryption.clone().unwrap_or_else(|| "none".into()).into());
...
fn build_tls(node: &ProxyNode, outbound: &mut Map<String, Value>) -> anyhow::Result<()> {
    match node.security {
        ...
        Security::Tls => {
            let mut tls = Map::new();
            tls.insert("serverName".into(), node.sni.clone().unwrap_or_else(|| node.server.clone()).into());
            ...
            outbound.insert("streamSettings".into(), serde_json::json!({ "security": "tls", "tlsSettings": tls }));
            Ok(())
        }
        Security::Reality | Security::RealityPsk => {
            let mut reality = Map::new();
            reality.insert("serverName".into(), node.sni.clone().unwrap_or_else(|| node.server.clone()).into());
    ...
    match node.transport {
        Transport::Ws | Transport::H2 | Transport::Xhttp | Transport::Splithttp => {
            ...
            if node.transport == Transport::Xhttp || node.transport == Transport::Splithttp {
                t.insert("path".into(), node.path.clone().unwrap_or_else(|| "/".into()).into());
...
mod tests {
    ...
    fn trojan_grpc_config_is_generated() {
        ...
        assert_eq!(stream.get("grpcSettings").unwrap().get("serviceName").unwrap(), "MyService");
    }
